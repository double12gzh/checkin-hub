...
class BasePlugin {
  ...
  async onSetup(context) {
    ...
    await page.waitForURL(this.url ? `**${new URL(this.url).pathname}*` : '**/', { timeout: 300000 });
  ...
  async onCheckin(context) {
    throw new Error(`Plugin [${this.name || this.id}] must implement the onCheckin(context) method.`);
  }
}
