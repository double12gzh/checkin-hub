# JustDoWork Plugin Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 实现并集成 JustDoWork (`https://api.justwoker.icu`) 自动签到与额度查询插件。

**Architecture:** 继承 `BasePlugin`，采用“Fast-Path 预检 + API 直接签到 + GitHub OAuth 自动重连 + DOM 回退”四层保障机制，严格遵循 Playwright 自动化八项铁律与时区/安全规范。

**Tech Stack:** Node.js, Playwright, Checkin-Hub 框架核心 (`core/base-plugin.js`, `core/helper.js`)。

## Global Constraints

- 插件必须位于 `plugins/justdowork.js`，继承 `BasePlugin`。
- ID: `justdowork`，名称: `JustDoWork`。
- 严禁滥用 `waitForTimeout`，严禁使用 `waitUntil: 'commit'`。
- 日期比对统一使用 CST（UTC+8）。
- 异常直接抛出 `new Error` 以便自动生成故障截图。
- 代码必须通过 ESLint 校验与 `scripts/verify-plugins.js` 契约检查。

---

### Task 1: 创建 JustDoWork 签到插件核心实现

**Files:**

- Create: `plugins/justdowork.js`

**Interfaces:**

- Consumes: `core/base-plugin.js:BasePlugin`, `core/helper.js:PageHelper`
- Produces: `JustDoWorkPlugin` class exporting `id: 'justdowork'`, `onCheckin`, `onSetup`

- [ ] **Step 1: 编写 `plugins/justdowork.js` 源码**

编写包含元数据定义、`onSetup` 交互式登录、`fetchUserData` 站内接口请求、`findTodayCheckin` CST时间匹配、`calculateBalance` 额度换算以及完整 `onCheckin` 打卡逻辑的代码。
...
预期输出：ESLint 校验与 3 项契约扫描全部 PASS。
...
预期输出：输出列表中包含 `justdowork`，状态为 Enabled。
...
```bash
git add plugins/justdowork.js docs/superpowers/plans/2026-09-18-justdowork-plugin.md
git commit -m "feat(justdowork): 新增 JustDoWork 自动签到与额度查询插件"
```
